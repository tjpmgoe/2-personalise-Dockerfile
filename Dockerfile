# White-label Flowise (painel + chat) — alvo: ${appTarget}
ARG FLOWISE_TAG=v3.0.5
ARG BRAND_NAME=SISAITONY
ARG APP_TARGET=flowise-teste-senha123456tony
...
RUN git clone https://github.com/FlowiseAI/Flowise.git . && (git checkout ${FLOWISE_TAG} || true)
...
WORKDIR /app
ENV PORT=3000
# Metadados do alvo (apenas rótulo para você identificar a imagem depois)
LABEL x-flowise-app-target="${APP_TARGET}"
COPY --from=build /src .
EXPOSE 3000
CMD ["pnpm","start"]